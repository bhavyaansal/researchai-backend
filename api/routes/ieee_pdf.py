...
# Rewritten spans are rendered identically to original text (no colour/italics)
# so the output is visually indistinguishable from the source document.
...
def _block_to_markup(block) -> str:
    # Rewritten spans are treated identically to unchanged text —
    # the output must be visually indistinguishable from the original.
    marked = []
    for text, _is_modified in block:
        marked.append(xml_escape(text))
