...
def _call_gemini(prompt: str) -> str:
    ...
    try:
        ...
        raise RuntimeError("Gemini API timed out after 30s — skipping this span")
    except requests.exceptions.RequestException as e:
        raise RuntimeError(f"Gemini API connection error: {e}")

    if response.status_code == 429:
        raise RuntimeError("Gemini rate limit (15 RPM) — skipping this span")
    if response.status_code == 404:
        raise RuntimeError(f"Gemini model not found: {response.text[:100]}")
    if response.status_code == 403:
        raise RuntimeError("Gemini API key invalid or quota exhausted")
    if response.status_code != 200:
        raise RuntimeError(f"Gemini error {response.status_code}: {response.text[:100]}")
...
def rewrite_paragraph(original_text: str, similarity_score: float) -> str:
    return _call_gemini(build_rewrite_prompt(original_text, similarity_score))


def rewrite_paragraph_retry(
    original_text: str, previous_attempt: str, previous_score: float
) -> str:
    return _call_gemini(build_retry_prompt(
